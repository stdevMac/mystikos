...
int main(int argc, const char* argv[])
{
    ...
    run_mode = argv[2];
    printf(
        "Host: Enclave path = %s, Run mode = %s, server port = %s\n",
        enclave_path,
        run_mode,
        SERVER_PORT);
    ...
    {
        printf(
            "Host: setup_tls_server failed with ecall return val: %d\n",
            ecall_ret);
        ret = -1;
        goto exit;
    }
...
exit:
    ...
    printf(
        "Host:  %s \n",
        (ret == 0) ? "oe_enclave app exited successfully"
                   : "oe_enclave app failed");
    return ret;
}
